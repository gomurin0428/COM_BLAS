...
## 1. 製品概要
COM_BLAS は、OpenBLAS をバックエンドに利用した数値線形代数ライブラリを COM コンポーネントとして提供します。ProgID は `Ckt.Com.Blas.BlasCore` で公開されており、.NET、VBA、C++ など COM Automation を扱える言語から倍精度実数および複素数の BLAS ルーチンを利用できます。本書は ComponentSource 経由で提供されるインストーラ パッケージ向けの利用ガイドです。
...
- `ReadMe.md`: 開発者向け技術ドキュメント（API 仕様やビルド手順）。
- `manual.md`: 本ドキュメント。
...
## 8. トラブルシューティング
代表的なエラーと対処法は `TROUBLESHOOTING.md` に集約されています。インストールや COM 登録で問題が発生した場合は同ドキュメントを参照し、再インストールや権限設定、`libopenblas.dll` の配置を確認してください。

## 9. サポート窓口
- 購入およびライセンス関連: ComponentSource のサポートチーム。
