...
- Type Library の物理名を `CktComBlas.tlb` に統一し (`COM_BLAS.vcxproj` の MIDL 出力名 / `COM_BLAS.rc` / `COM_BLAS.Setup.vdproj`)、`BLAS.rgs` で旧 `COMBLASLib` / `COMBLAS.BLAS` の ProgID および `HKCR\TypeLib\{5650...}\1.*` を ForceRemove してから再登録するよう調整。`COM_BLAS_UnitTest_Managed.csproj` の COM 参照を `CktComBlasLib` v1.5 へ更新し、関連ドキュメント (`ReadMe.md`, `TROUBLESHOOTING.md`, `IBLASComplex_new_api_definitions.md`, `COMBLASComplex_recovery_plan.md`, `makingInstallerPlan.md`, `TEXT_FILE_OVERVIEW.md`, `残作業.md`) を改名後の名称で整備。`msbuild COM_BLAS.sln /t:COM_BLAS /p:Configuration=Debug /p:Platform=x64` を再実行し成功を確認。
